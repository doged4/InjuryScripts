...
for i in $(ls *2.fastq.gz | rev | cut -c 12- | rev | uniq);  do\
       ...
       /STAR/bin/Linux_x86_64/STAR --runMode alignReads --runThreadN 50 --genomeDir /genome/genomes/Ensembl_Grch38/STAR.Ensembl.GRCh38   --readFilesIn  $thisdir/${i}_1.fastq.gz   $thisdir/${i}_2.fastq.gz   --outFileNamePrefix $thisdir/STAR.Ensembl.Out/${i}/  --sjdbGTFfile /genome/genomes/Ensembl_Grch38/Homo_sapiens.GRCh38.100.gtf --outSAMtype SAM --readFilesCommand zcat;
cd ..; cd ..;
	done;
