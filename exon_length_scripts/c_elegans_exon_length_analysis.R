library(readr)
celegans_coordinates <- read_delim("Documents/exon_length_analysis/c_elegans_exon_work.txt", 
                                      delim = "\t", escape_double = FALSE, 
                                      trim_ws = TRUE)

Exons_with_lens <- celegans_coordinates
rm(celegans_coordinates)
...
interesting_genes <- gene_statistics_multiexons %>% filter(`count_a` >= 2) %>%
  filter(`count_b` >= 1)
