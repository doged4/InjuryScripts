library(readr)
Exon_coordinates_GRCh38 <- read_delim("Documents/exon_length_analysis/chicken_exon_analysis.txt", 
...
Exons_with_lens %>% ggplot(aes(x=length)) + geom_histogram(bins = 50) 
    + xlim(c(0,500)) 
    + xlab("exon length") 
    + ggtitle("Exon length across whole genome")
...
data.frame( counts_for_twos) %>% ggplot(aes(x=counts_for_twos)) + geom_histogram(binwidth = 1)
